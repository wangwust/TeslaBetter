...
namespace Tesla.Service
{
    ...
    public class LHCClientWork
    {
        ...
        private void Run()
        {
            ...
            {
                TeslaHelper.WriteLog(appTask.ID, appTask.Name, LogTypeEnum.ERROR, $"[客户端]登录失败。信息" + response.msg, SourceEnum.Client, appTask.ClientUserName);
                return;
            }
        ...
        private bool Bet(AppTask task, BetParams param)
        {
            ...
            {
                TeslaHelper.WriteLog(task.ID, task.Name, LogTypeEnum.ERROR, $"[客户端]投注失败。参数有误。实体：{param.ToJson()}", SourceEnum.Client, task.ClientUserName);
                return true;
            }
            ...
            try
            {
                ...
                if (response.IsSucceed)
                {
                    ...
                    if (balance < task.SingleMoney * (49 - task.ServerMinNumCount))
                    {
                        ...
                        TeslaHelper.WriteLog(task.ID, task.Name, LogTypeEnum.INFO, $"[客户端]余额仅剩{balance}，不足下次投注，已将任务停止。", SourceEnum.Client, task.ClientUserName);
                    }
                ...
                if (response.msg.ToLower().Contains("token"))
                {
                    ...
                    if (newLogin.IsSucceed)
                    {
                        TeslaHelper.WriteLog(task.ID, task.Name, LogTypeEnum.INFO, $"[客户端]当前用户已掉线，已经重新登录", SourceEnum.Client, task.ClientUserName);
                        this._loginResponse = newLogin.data;
                        return false;
                    }
                    else
                    {
                        ...
                        TeslaHelper.WriteLog(task.ID, task.Name, LogTypeEnum.INFO, $"[客户端]当前用户已掉线，重新登录失败，已将任务：{task.Name}停止", SourceEnum.Client, task.ClientUserName);
                        return true;
                    }
                }
