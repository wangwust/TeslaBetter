...
namespace Tesla.Service
{
    ...
    public class LHCServerWork
    {
        ...
        private void Run()
        {
            ...
            {
                TeslaHelper.WriteLog(appTask.ID, appTask.Name, LogTypeEnum.ERROR, $"[服务端]登录失败，用户名：{appTask.ServerUserName}。信息" + response.msg, SourceEnum.Server, appTask.ServerUserName);
                return;
            }
        ...
        private bool Bet(AppTask task, string issueNo)
        {
            ...
            if (!response.IsSucceed)
            {
                ...
                else if (response.msg.ToLower().Contains("token"))
                {
                    ...
                    if (newLogin.IsSucceed)
                    {
                        TeslaHelper.WriteLog(task.ID, task.Name, LogTypeEnum.INFO, $"[服务端]当前用户已掉线，已经重新登录", SourceEnum.Server, task.ServerUserName);
                        this._loginResponse = newLogin.data;
                    }
                    else
                    {
                        TeslaHelper.StopTask(task.ID, TaskStopReason.ServerToken);
                        TeslaHelper.WriteLog(task.ID, task.Name, LogTypeEnum.INFO, $"[服务端]当前用户已掉线，重新登录失败，已将任务：{task.Name}停止", SourceEnum.Server, task.ServerUserName);
                    }
                }
            }
            else
        ...
        private ApiResponse<LoginResponse> Login(AppTask task)
        {
            ...
            while (index < loops)
            {
                ...
                {
                    TeslaHelper.WriteLog(task.ID, task.Name, LogTypeEnum.ERROR, $"[服务端]用户登录异常，用户：{task.ServerUserName}。详情：{ex.ToString()}", SourceEnum.Server, task.ServerUserName);
                    index++;
                    Thread.Sleep(10 * 1000);
                }
            }
